...
impl BrightnessBlock {
    ...
    fn update_max_brightness(&mut self) -> Result<(), UpdateError> {
        self.max_brightness = match utils::get_int_from_file(&PathBuf::from(BASE_DIR).join(&self.card).join("max_brightness")) {
            Ok(b) => b,
    ...
    fn update_current_brightness(&mut self) -> Result<(), UpdateError> {
        self.current_brightness = match utils::get_int_from_file(&PathBuf::from(BASE_DIR).join(&self.card).join("/brightness")) {
            Ok(b) => b,
            Err(e) => return Err(UpdateError {
                block_name: self.name().to_string(),
                message: format!("couldn't get max brightness: {}", e)
...
fn get_icon(percentage: i32) -> char {
    let index = ((percentage * BRIGHTNESS_ICONS.len() as i32 / 100) as usize).min(BRIGHTNESS_ICONS.len() - 1);
