...
use crate::{
    format::{color::RGBA, Attention, Formatter},
    utils,
};
...
pub struct BlockOutput {
    ...
    /// The Attention level of the output, which may give the block a special
    /// color.
    attention: Attention,
}
...
impl BlockOutput {
    ...
    /// Formats the output as a pango string. The first string returned is the
    /// full text including icon, primary text, and secondary text. The
    /// second string is the same but excludes the secondary text.
    pub fn as_pango_strings(&self, f: &Formatter) -> (String, Option<String>) {
        ...
        if let Some(icon) = icon_pango {
            ...
            let short_with_icon = if let Some(short) = short_opt {
                ...
                format!("{icon} {short}") // i accidentally only put one space
                                          // in here, but idk,
...
/// Text that is displayed with a block, either a single string or a primary and
/// secondary string.
///
/// When displaying primary and secondary data, the two strings are colored
/// differently. The intent is to color the primary and secondary text
/// differently such that the primary text is more prominent as it's the more
...
impl BlockText {
    /// Returns the long and short versions of the `BlockText` in pango markup
    /// format.
    ///
    /// If `Single`, the long version is the string and the short version is
    /// `None`.
    ///
    /// If `Pair`, the long version is both strings, and the short version is
    /// only the primary string.
    ///
    /// TODO: Add `to_pango_strings_with_icon` to include an icon with the
    /// primary text and reduce the amount of markup we have to do
    fn to_pango_strings(
        &self,
        primary_color: RGBA,
        ...
        let second_half = match self {
            BlockText::Single(s) | BlockText::Pair(_, s) => {
                let color = match self {
                    ...
                    BlockText::Pair(..) => secondary_color,
                };
    ...
    /// Returns the short version of the pango markup representation of this
    /// `BlockText`.
    ///
    ...
    /// Returns the long version of the pango markup representation of this
    /// `BlockText`.
    ///
    /// If `Single`, the long version is the single string.
    ///
    /// If `Pair`, the long version is both strings, colored differently to
    /// accent the 'primary' text.
    #[allow(dead_code)]
    fn to_long_pango_string(&self, primary_color: RGBA, secondary_color: RGBA) -> String {
        ...
        //
        // if we moved the logic of `to_pango_strings` here, `to_pango_strings` would
        // require two calls to `to_short_pango_string`: once to get the short
        // text, and another time to create the long text here.
        //
        // by having the string-building logic in `to_pango_strings`,
        // `to_short_pango_string` only has to be called once if we wanted to
        // create *both* short and long versions of the `BlockText`.
        //
        // `to_short_pango_string` has to be called anyway to create the long string, so
        // there's no harm in having it called in `to_pango_strings`.
